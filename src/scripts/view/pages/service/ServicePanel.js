...
import { wallet_checkout, edit_wallet } from "../../../core/actions/wallet";
import {
  uploadFile,
...
const ServicePanel = ({ dispatch, match, auth, service, comment }) => {
  const [editOpen, setEditOpen] = useState(false);
  const [showCheckoutModal, setCheckoutModalOpen] = useState(false);
  ...
  const handleUploadFile = (file) => {
    dispatch(uploadFile(file, service.serviceDetail.id, "business"));
  };
  ...
  const handleCheckoutSubmit = () => {
    dispatch(
      wallet_checkout(
        auth.user.id,
        service.serviceDetail.service.wallet_id,
        checkoutValue,
        editWalletSuccess
      )
    );
  ...
  const editWalletSuccess = () => {
    dispatch(
      get_service(match.params.serviceID, firstOfWeek.format("YYYY-MM-DD"))
    );
  };
  const handleEditWalletSubmit = () => {
    dispatch(
      edit_wallet(
        auth.user.id,
        service.serviceDetail.service.wallet_id,
      ...
      {service.serviceLoadStatus === "loaded" && (
        <div style={{ padding: "1em" }}>
          <EditServiceForm
            ...
            nameValue={service.serviceDetail.service.name}
            descriptionValue={service.serviceDetail.service.description}
            addressValue={service.serviceDetail.service.address}
            priceValue={service.serviceDetail.service.fee}
            cancellationRangeValue={
              service.serviceDetail.service.cancellation_range
            }
          />
          <Modal
            title="رزرو سانس"
          ...
          >
            <Form>
              <Form.Item label="میزان  برداشت از حساب">
                <Input
                  value={checkoutValue}
          ...
          <Modal
            title="ویرایش اسم کیف پول"
            visible={showWalletEditModal}
            closable={false}
            okText="بله"
            cancelText="خیر"
            onOk={() => {
              handleEditWalletSubmit();
            }}
            onCancel={() => {
              setShowWalletEditModal(false);
              setWalletName("");
            }}
            open={showWalletEditModal}
            onClose={() => setShowWalletEditModal(false)}
          >
            <Form>
              <Form.Item label="اسم کیف پول">
                <Input
                  value={walletName}
              ...
              <Card
                actions={[
                  ...
                  <SettingOutlined
                    key="setting"
                    onClick={() => {
                      setWalletName(service.serviceDetail.wallet.name);
                      setShowWalletEditModal(true);
                    }}
                  />,
                ]}
              >
                <Card.Meta
                  title={service.serviceDetail.wallet.name}
                  description={`اعتبار: ${service.serviceDetail.wallet.credit}`}
                />
              </Card>
            </Col>
          </Row>
...
export default connect(mapStataToProps)(withRouter(ServicePanel));
